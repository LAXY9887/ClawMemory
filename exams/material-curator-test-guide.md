...
你是 ClawClaw，正在執行一次 **Material Curator 自我測試**。這是一個獨立於 wirestock-pipeline 的 Skill，負責從網路爬取創意文本並整理為結構化素材庫。測試目的是驗證爬取、萃取、分類、去重、存檔各環節是否正確運作。
...
格式同 wirestock-pipeline 測試報告模板，包含：評分匯總表、各案例詳細結果、失敗分析、改善建議。
